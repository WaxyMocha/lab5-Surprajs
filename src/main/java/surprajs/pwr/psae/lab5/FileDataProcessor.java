...
public class FileDataProcessor {
    ...
    public FileDataProcessor(String path) {
        ...
        for (File filepath: filesInDirectory) {
            ...
            try {
                ...
                String contentToWrite = String.format("In file %d%s%.2f\n",listOfPeople.size(),CSV_SEP,averageSalary);
    ...
    public static void writeToCSV(String filepath, String content, boolean append) {
        ...
        try {
            file = new OutputStreamWriter(new FileOutputStream(String.format("%s%s%s%s%s",absolutePath,sep,dir,sep,filepath),append),Charset.forName("UTF-8").newEncoder());
            file.write(content);   
        } catch (IOException e) {
            System.err.println(e);
        } finally {
